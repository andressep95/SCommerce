...
-- Crear trigger para registrar cambios en productos
DROP TRIGGER IF EXISTS trg_log_product_changes ON products;
CREATE TRIGGER trg_log_product_changes
    AFTER INSERT OR UPDATE OR DELETE ON products
    FOR EACH ROW EXECUTE FUNCTION log_product_changes();

-- Crear función para registrar cambios en cotizaciones
CREATE OR REPLACE FUNCTION log_quotation_changes() RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'UPDATE' THEN
...
-- Crear trigger para registrar cambios en cotizaciones
DROP TRIGGER IF EXISTS trg_log_quotation_changes ON quotations;
CREATE TRIGGER trg_log_quotation_changes
